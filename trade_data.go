...
// TradeData represents the response structure for trade data queries
...
func (srv *Client) GetTradeDataSingle(address string) (*TradeData, error) {
	url := fmt.Sprintf("%s/v3/token/trade-data/single?address=%s", baseURL, address)
	return newCli[TradeData](srv.apiKey).makeRequest("GET", url, nil)
}
